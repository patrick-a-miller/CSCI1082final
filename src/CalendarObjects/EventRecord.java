...
public class EventRecord {
	...
	public static Comparator<EventRecord> compareDateRoom = new Comparator<EventRecord>() {
		...
		private String combineValues(int year, int month, int day, Room room, int hour) {
			...
			String text = Integer.toString(year) + monthPad + Integer.toString(month) + dayPad + Integer.toString(day)
					+ room.getRoomId() + hourPad + Integer.toString(hour);
			return text;
		}
	...
	public EventRecord(int year, int month, int day, Room room, int hour, ClassEntry classEntry, Teacher teacher) {
		isValid = true;
		isValid = setYear(year) && isValid;
		isValid = setMonth(month) && isValid;
		isValid = setDay(day) && isValid;
		isValid = setRoom(room) && isValid;
		isValid = setHour(hour) && isValid;
		isValid = setClassEntry(classEntry) && isValid;
		isValid = setTeacher(teacher) && isValid;
